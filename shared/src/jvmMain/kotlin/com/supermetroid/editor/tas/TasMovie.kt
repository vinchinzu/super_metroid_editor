...
class TasMovie(
    ...
    fun withMeta(meta: TasMovieMeta): TasMovie = TasMovie(meta, frames, trace)

    fun withTrace(trace: List<TasTracePoint>): TasMovie = TasMovie(meta, frames, trace)

    /** Truncate to [length] frames (rerecord from a mid-movie state). */
    fun truncated(length: Int): TasMovie = TasMovie(meta, frames.take(length), trace.filter { (it.frame ?: 0) < length })

    fun appended(frame: IntArray): TasMovie = TasMovie(meta, frames + listOf(frame), trace)

    fun appendedAll(newFrames: List<IntArray>): TasMovie = TasMovie(meta, frames + newFrames, trace)
    ...
    fun spliced(start: Int, replacement: List<IntArray>): TasMovie {
        ...
        return TasMovie(meta, result, trace)
    }
    ...
    companion object {
        ...
        fun fromJson(text: String): TasMovie {
            ...
            return TasMovie(doc.meta, doc.frames.map { TasInput.decodeFrame(it) }, doc.trace)
        }
