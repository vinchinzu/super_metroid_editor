...
def noop_action(player_count: int = 1) -> list[int]:
    return [0] * (ACTIONS_PER_PLAYER * max(1, int(player_count)))
...
class SeedRouteDriver:
    def __init__(self, buttons: list[list[int]]) -> None:
        self._buttons = [
            list(frame[:ACTIONS_PER_PLAYER]) + [0] * max(0, ACTIONS_PER_PLAYER - len(frame))
...
class Session:
    ...
    def __post_init__(self) -> None:
        noop = noop_action(self.player_count)
