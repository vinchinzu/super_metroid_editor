...
def _input_imports():
    import pygame
    from controls import controller_action, init_controllers, sanitize_action_multi

    return pygame, controller_action, init_controllers, sanitize_action_multi
...
class BridgeRuntime:
    def __init__(self) -> None:
        ...
        self.player_count = DEFAULT_PLAYER_COUNT
        self._pygame = None
        self._controllers: list[Any] = []
        self._controller_action = None
        self._init_controllers = None
    ...
    def _apply_config(
        ...
        *,
        ...
        if player_count is not None:
            self.player_count = self._clamp_player_count(player_count)
            if self._pygame is not None:
                self._refresh_controllers(force=True)
    ...
    @staticmethod
    def _clamp_player_count(player_count: int | None) -> int:
        try:
    ...
    @staticmethod
    def _normalize_action(action: list[int], player_count: int) -> list[int]:
        size = ACTIONS_PER_PLAYER * player_count
        requested = [int(v) for v in action[:size]]
        if len(requested) < size:
            requested.extend([0] * (size - len(requested)))
        return requested

    @staticmethod
    def _expand_single_player_action(action: list[int], player_count: int) -> list[int]:
        requested = list(action[:ACTIONS_PER_PLAYER])
        if len(requested) < ACTIONS_PER_PLAYER:
            requested.extend([0] * (ACTIONS_PER_PLAYER - len(requested)))
        if player_count <= 1:
            return requested
        return requested + noop_action(player_count - 1)
    ...
    def hello(self) -> dict[str, Any]:
        return {
            "capabilities": {
                ...
                "maxPlayers": 2,
                "defaultPlayerCount": self.player_count,
            },
    ...
    def _ensure_input_runtime(self) -> None:
        if self._pygame is not None:
            return
        try:
            pygame, controller_action, init_controllers, sanitize_action = _input_imports()
            pygame.init()
        except Exception:
            return
        self._pygame = pygame
        self._controller_action = controller_action
        self._init_controllers = init_controllers
        self._sanitize_action = sanitize_action
        self._refresh_controllers(force=True)

    def _refresh_controllers(self, *, force: bool = False) -> None:
        if self._pygame is None or self._init_controllers is None:
            return
        now = time.time()
        ...
        try:
            ...
            pass
        try:
            controllers = self._init_controllers(self._pygame)
        except Exception:
            controllers = []
        expected_players = self.session.player_count if self.session is not None else self.player_count
        self._controllers = list(controllers[: expected_players])
        names = [controller.get_name() for controller in self._controllers]
        self._controller_name = ", ".join(names) if names else None

    def _controller_status(self) -> tuple[bool, str | None]:
        self._ensure_input_runtime()
        self._refresh_controllers()
        return bool(self._controllers), self._controller_name
    ...
    def _resolve_action(self, session: Session, action: list[int]) -> ResolvedActionInfo:
        requested = self._normalize_action(action, session.player_count)
        noop = noop_action(session.player_count)
        ...
        if self.control_mode == "agent":
            if session.model_driver is not None and session.last_obs is not None:
                try:
                    ...
                    expanded = self._expand_single_player_action(predicted, session.player_count)
                    applied = list(expanded)
                    if self._sanitize_action is not None:
                        self._sanitize_action(applied, players=session.player_count)
                    return ResolvedActionInfo(
                        requested=list(expanded),
                        pre_sanitize=list(expanded),
                        applied=applied,
                        source="agent_model",
        ...
        if self.control_mode == "manual" and self._pygame is not None:
            ...
            if self._controller_action is not None:
                for index, controller in enumerate(self._controllers[: session.player_count]):
                    try:
                        self._controller_action(controller, resolved, offset=index * ACTIONS_PER_PLAYER)
                    except Exception:
                        self._controllers = []
        ...
        if self._sanitize_action is not None:
            self._sanitize_action(resolved, players=session.player_count)
        return ResolvedActionInfo(
            requested=requested,
    ...
    def session_state(self) -> dict[str, Any]:
        ...
        return {
            ...
            "playerCount": self.session.player_count,
        }
    ...
    def start_session(
        ...
        *,
        ...
        noop = noop_action(player_count)
        env = make_env(GAME, state_name, EDITOR_DIR, render_mode="rgb_array", players=player_count)
        env.reset()
    ...
    def _finalize_recording(self, session: Session) -> tuple[Path | None, Path | None]:
        ...
        metadata = {
            ...
            "action_stride": ACTIONS_PER_PLAYER,
